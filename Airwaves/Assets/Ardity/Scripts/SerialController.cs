...
public class SerialController : MonoBehaviour {
	...
	void Update() {
		...
		if (ReferenceEquals(message, SERIAL_DEVICE_CONNECTED))
			print("Connect success " + my_input + "\n");
		else if (ReferenceEquals(message, SERIAL_DEVICE_DISCONNECTED))
			print("Connect failed " + my_input + "\n");
		else {
			if (my_input == InputSource.Enigma) {
				messageListener.SendMessage("OnMessageArrived", message);
			} else if (my_input == InputSource.Phone) {
				messageListener.SendMessage("OnPhoneArrived", message);
			} else if (my_input == InputSource.Antenna_L) {
				messageListener.SendMessage("OnLeftAntennaArrived", message);
			} else if (my_input == InputSource.Antenna_R) {
				messageListener.SendMessage("OnRightAntennaArrived", message);
			}
		}
	}
